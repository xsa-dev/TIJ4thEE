...
public class tij0310 {
    ...
    public static void main(String[] args) {
        ...
        int c = a & b;     //Поразрядный оператор И - & - заносит 1 в выходной бит, если оба выходных бита равны 1, иначе заносит 0
        System.out.println("Поразрядный оператор И от a к b: " + Integer.toBinaryString(c));
        c = a | b; //Поразрядный оператор ИЛИ - | - заносит 1 в выходной бит, если хотя бы один из битов операндов был равен 1, результат равен 0 только в том случае когда оба бита операндов были нулевыми
        System.out.println("Поразрядный оператор ИЛИ от a к b: " + Integer.toBinaryString(c));
        c = a ^ b; //Поразрядный оператор ИСКЛЮЧАЮЩЕ (XOR, ^), имеет результат 1 тогда, когда один или другой из входных битов был единицей, но не оба вместе
        System.out.println("Поразрядный оператор ИСКЛЮЧАЮЩЕ (XOR, ^) от a к b: " + Integer.toBinaryString(c));
        c = ~a; // Поразрядный оператор НЕ (оператор Двоичного Дополнения) является унарным оператором, то есть имеет только один операнд. Поразрядное НЕ производит бит, противоположный исходному - если входящий бит является нулём, то в результате будет еденица, если вхожящий единица, то в результате будет ноль
